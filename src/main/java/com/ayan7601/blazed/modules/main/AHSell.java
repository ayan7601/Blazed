...
import com.ayan7601.blazed.utils.BlazedSell;
...
public class AHSell extends Module {
    ...
    public AHSell() {
        super(BlazedAddon.CATEGORY, "ah-sell", "Automatically sells all hotbar items using /ah sell.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (BlazedSell.isDialogOpen()) {
            if (BlazedSell.clickDialogYes()) {
        ...
        if (screenHandler instanceof ChestMenu handler) {
            if (handler.getRowCount() == 3) {
                ...
                int confirmSlot = handler.getSlot(15).getItem().isEmpty()
                    ? BlazedSell.findConfirmSlot(handler)
                    : 15;
