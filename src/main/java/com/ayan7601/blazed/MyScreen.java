...
public class MyScreen extends WindowScreen {
    ...
    private static final Logger LOG = LoggerFactory.getLogger("Blazed");
    ...
    private static String fetchVersion() {
        try {
            String versionString = Http.get(BlazedAddon.VERSION_CHECK_URL).sendString();
    ...
    private static boolean isNewerThanInstalled(String latest) {
        return compareVersions(latest, BlazedAddon.VERSION) > 0;
    }
    ...
    private void buildUI() {
        ...
        addMessage(String.format("Installed Version: %s", BlazedAddon.VERSION));
        ...
        githubButton.action = () -> Util.getPlatform().openUri("https://github.com/ayan7601/blazed");

        WButton websiteButton = buttonsContainer.add(theme.button("Website")).expandX().widget();
        websiteButton.action = () -> Util.getPlatform().openUri("https://blazedclient.com");
    }
